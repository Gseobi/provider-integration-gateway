...
결제 요청 분기 · Provider별 요청 데이터 구성 · 공통 응답 표준화를  
Backend 게이트웨이 책임으로 분리한 포트폴리오 프로젝트입니다.

실제 운영 환경에서 사용했던 PG 분기 전략, 책임 분리 구조, 결제 요청 흐름을 바탕으로  
보안 및 계약 제약을 고려해 **Mock 기반 구조**로 재구성했습니다.
...
PG 선택 / 분기 / 요청 데이터 구성 책임을 Backend 게이트웨이로 명확히 분리한 구조를  
Mock 기반으로 재현합니다.
...
- PG 선택 책임을 Backend 게이트웨이 계층으로 집중할 수 있습니다.
- Provider별 요청 데이터 구성과 응답 차이를 Strategy 기반으로 분리할 수 있습니다.
- 클라이언트는 Provider별 세부 구현 대신 공통 Response DTO 기준으로 처리할 수 있습니다.
- 신규 PG 추가 시 Controller 수정 없이 Strategy 추가 중심으로 확장할 수 있습니다.
- 실제 계약/보안 제약이 있는 외부 연동도 Mock 기반으로 구조와 책임을 설명할 수 있습니다.
...
3. PaymentOrchestrator가 PG 코드 기준으로 적절한 Provider Strategy를 선택합니다.
...
|---|---|---|---|
| Provider branching | PG 코드 기준으로 적절한 Strategy 선택 | Pass | `docs/test-report.md` |
| Provider request building | Provider별 요청 데이터가 분리되어 구성 | Pass | `docs/test-report.md` |
...
- **Client(App)**  
  결제 요청을 Backend 게이트웨이에 전달

- **PaymentController**  
  요청 진입점, 검증 및 서비스 호출

- **PaymentOrchestrator**  
  PG 코드 기준 Provider Strategy 선택
...
- **Response DTO**  
  Provider별 차이를 흡수한 공통 응답 구조

<br/>

## 8. Request / Response Strategy
...
본 프로젝트에서 확인하는 주요 시나리오는 다음과 같습니다.
...
이 프로젝트는 다수의 PG 연동 환경에서  
결제 요청 분기, Provider별 요청 데이터 구성, 공통 응답 표준화 책임을  
Backend 게이트웨이 계층으로 분리해 설명하는 포트폴리오 프로젝트입니다.

실제 결제 승인 구현이 아니라,  
PG 선택 책임의 집중, Strategy 기반 분리, 클라이언트 결합도 완화,  
신규 Provider 확장 가능 구조를 중심으로 설계했다는 점에 의미가 있습니다.
